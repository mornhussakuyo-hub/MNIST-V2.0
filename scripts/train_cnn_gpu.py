...
from cnn_model_gpu import TwoConvNetGPU
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the handwritten two-convolution MNIST model on CUDA GPU")
    ...
    parser.add_argument("--model-name", type=str, default="cnn_gpu_model", help="Model name")
    ...
    parser.add_argument("--conv1-filters", type=int, default=16, help="First convolution filters")
    parser.add_argument("--conv2-filters", type=int, default=32, help="Second convolution filters")
    parser.add_argument("--val-ratio", type=float, default=0.2, help="Proportion for validation set")
    parser.add_argument("--usage-ratio", type=float, default=1.0, help="Proportion used from training data")
    parser.add_argument("--reg-rate", type=float, default=0.0, help="L2 regularization rate")
    parser.add_argument("--gpu", type=int, default=0, help="CUDA GPU id")
    parser.add_argument("--eval-batch-size", type=int, default=1024, help="Batch size used for accuracy evaluation")
    parser.add_argument("--metric-samples", type=int, default=10000, help="Training samples used for per-epoch train accuracy; 0 means all")
...
def to_gpu_labels(labels):
    return cp.asarray(one_hot_encode(labels).astype(np.float32))
...
def main():
    ...
    print("\nCUDA CNN training start...")
    print(f"GPU: {args.gpu} - {cp.cuda.runtime.getDeviceProperties(args.gpu)['name'].decode()}")
    ...
    train_labels = to_gpu_labels(train_labels)
    val_labels = to_gpu_labels(val_labels)
    test_labels = to_gpu_labels(test_labels)
    ...
    print("\nStart CUDA CNN training.")
    for epoch in range(args.epochs):
        start_time = time.time()
        indices = cp.random.permutation(num_train)
        epoch_loss = 0.0
        num_batches = 0
        ...
        for start in range(0, num_train, args.batch_size):
            ...
            epoch_loss += scalar(loss)
            num_batches += 1

        avg_train_loss = epoch_loss / num_batches

        if args.metric_samples and args.metric_samples < num_train:
            metric_indices = cp.random.choice(num_train, args.metric_samples, replace=False)
            metric_images = train_images[metric_indices]
            metric_labels = train_labels[metric_indices]
        else:
            metric_images = train_images
            metric_labels = train_labels

        train_acc = model.accuracy(metric_images, metric_labels, batch_size=args.eval_batch_size)
        val_probs = model.forward(val_images)
        val_loss = scalar(model.compute_loss(val_probs, val_labels, args.reg_rate))
        val_acc = model.accuracy(val_images, val_labels, batch_size=args.eval_batch_size)
        synchronize()
        ...
        metric_note = f"{metric_images.shape[0]} samples" if metric_images.shape[0] != num_train else "all samples"
        print(
            f"Epoch: {epoch + 1:3d}/{args.epochs} | "
            f"Loss: {avg_train_loss:.4f} (train)/{val_loss:.4f} (valid) | "
            f"Acc: {train_acc * 100:.2f}% (train, {metric_note})/{val_acc * 100:.2f}% (valid) | "
            f"Time: {elapsed:.1f}s"
        )

    print("\nRunning on test data...")
    test_acc = model.accuracy(test_images, test_labels, batch_size=args.eval_batch_size)
    ...
    results_path = results_dir / f"{args.model_name}_cnn_gpu_training_history.npz"
    np.savez(
        results_path,
    ...
    print("\nGenerating CUDA CNN training visualizations...")
    ...
    print(f"CUDA CNN model has been saved at: {model_path}")
