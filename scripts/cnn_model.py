...
class Conv2D:
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, pad=1):
        ...
        self.W = (np.random.randn(out_channels, in_channels, kernel_size, kernel_size) * scale).astype(np.float32)
...
class MaxPool2D:
    ...
    def backward(self, dout):
        ...
        dmax = np.zeros((rows, channels, pool_size), dtype=dout.dtype)
        dmax[np.arange(rows)[:, None], np.arange(channels)[None, :], self.arg_max] = dout_flat
...
class TwoConvNet:
    def __init__(self, model_name="cnn_model", conv1_filters=8, conv2_filters=16):
        ...
        self.W3 = (np.random.randn(flattened_size, 10) * np.sqrt(2.0 / flattened_size)).astype(np.float32)
    ...
    def forward(self, x):
        ...
        scores = self.flat_input @ self.W3 + self.b3
    ...
    def compute_loss(self, probs, y, reg_lambda=0.0):
        ...
        data_loss = -np.sum(y * np.log(clipped)) / y.shape[0]
        reg_loss = 0.5 * reg_lambda * (
            np.sum(self.conv1.W * self.conv1.W) +
            np.sum(self.conv2.W * self.conv2.W) +
    ...
    def backward(self, x, y, reg_lambda=0.0):
        ...
        self.grads["b3"] = np.sum(dout, axis=0)
    ...
    def load(cls, filepath):
        ...
        model.conv1.W = data["conv1_W"]
        model.conv1.b = data["conv1_b"]
        model.conv2.W = data["conv2_W"]
        model.conv2.b = data["conv2_b"]
        model.W3 = data["W3"]
        model.b3 = data["b3"]
        return model
